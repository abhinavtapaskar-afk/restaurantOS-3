...
import { UtensilsCrossed, Image as ImageIcon, Plus, Minus, ShoppingCart, X, Star } from 'lucide-react';
            ...
            <div className={cn("fixed top-0 right-0 h-full w-full max-w-sm bg-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out z-50", isOpen ? 'translate-x-0' : 'translate-x-full')}>
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-4 border-t border-slate-800 space-y-4">
                        ...
                        <button onClick={() => setIsCheckoutOpen(true)} className="w-full bg-emerald-600 text-white font-bold py-3 rounded-lg hover:bg-emerald-500 transition-colors">
                            Proceed to Checkout
                        </button>
                    </div>
                    </>
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title="Checkout">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={isPlacingOrder} className="w-full bg-emerald-600 text-white font-bold py-3 rounded-lg hover:bg-emerald-500 disabled:bg-slate-700">
...
const PublicMenuPageContent: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (restaurantError || !restaurantData) { setError("Restaurant not found."); } 
            ...
            const { data: reviewData } = await supabase.from('reviews').select('*').eq('restaurant_id', restaurantData.id).order('created_at', { ascending: false });
        ...
        <div className="min-h-screen bg-slate-950 text-slate-300">
            <header className="h-96 bg-cover bg-center relative" style={{ backgroundImage: `url(${restaurant.hero_image_url || '/placeholder-hero.jpg'})` }}>
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-center p-4">
                    <h1 className="text-5xl md:text-7xl font-bold text-white">{restaurant.name}</h1>
                    <p className="text-lg text-slate-300 mt-2">{restaurant.city}</p>
                </div>
            </header>
            
            <main className="max-w-5xl mx-auto p-4 sm:p-6 md:p-8">
                {restaurant.about_us && (
                    <section className="bg-slate-900 p-8 rounded-lg -mt-20 relative z-10 border border-slate-800 shadow-xl">
                ...
                <section className="mt-12">
                    <h2 className="text-3xl font-bold text-emerald-400 text-center mb-8">Our Menu</h2>
                                ...
                                {items.map(item => (
                                    <div key={item.id} className="bg-slate-900 p-4 rounded-lg border border-slate-800 flex gap-4">
                                        ...
                                        <div className="flex flex-col flex-grow">
                                            ...
                                            <button onClick={() => addToCart(item)} className="self-end -mb-2 -mr-2 bg-emerald-600 text-white rounded-full p-2 hover:bg-emerald-500 transition-colors"><Plus size={20} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <section className="mt-12">
                     <h2 className="text-3xl font-bold text-emerald-400 text-center mb-8">What Our Customers Say</h2>
                        ...
                        {reviews.map(review => (
                            <div key={review.id} className="bg-slate-900 p-6 rounded-lg border border-slate-800">
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex gap-1">{[...Array(review.rating)].map((_, i) => <Star key={i} size={16} className="text-yellow-400 fill-yellow-400" />)}</div>
                                </div>
            ...
            <footer className="text-center p-8 mt-8 border-t border-slate-800 text-slate-500">
                <p>{restaurant.address}</p>
                <p>{restaurant.phone_number}</p>
            </footer>

            <button onClick={() => setIsCartOpen(true)} className="fixed bottom-6 right-6 bg-emerald-600 text-white rounded-full p-4 shadow-lg hover:bg-emerald-500 transition-transform hover:scale-110">
