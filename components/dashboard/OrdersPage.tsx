                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-800">
                        <thead className="bg-slate-800">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Order ID</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Time</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Customer</th>
                            ...
                            {orders.map((order) => (
                                <tr key={order.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={twMerge("px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full", getStatusClass(order.status))}>
                                            {order.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                       <div className="flex gap-2">
                                            {order.status === 'pending' && (
                                                <button onClick={() => updateStatus(order.id, 'preparing')} className="text-blue-400 hover:text-blue-300 bg-blue-500/10 px-3 py-1 rounded-md">Prepare</button>
                                            )}
                                            {order.status === 'preparing' && (
                                                 <button onClick={() => updateStatus(order.id, 'delivered')} className="text-emerald-400 hover:text-emerald-300 bg-emerald-500/10 px-3 py-1 rounded-md">Deliver</button>
                                            )}
                                       </div>
                                    </td>
                                </tr>
